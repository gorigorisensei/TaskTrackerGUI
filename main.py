...
def main_menu():
    ...
    global window
    global pf_button, config_button, week_tasks_button
    ...
    config_button.place(x=config_x - 100, y=button_y + 100)
    ...
    pf_button.place(x=config_x + 250, y=button_y + 100)
    week_tasks_button = customtkinter.CTkButton(text="View Weekly Task List", command=list_weeks_tasks,
                                        hover_color="#3610B6", text_font=icon_font)
    week_tasks_button.place(x=config_x + 680, y=button_y + 100)

    window.mainloop()
def task_to_csv(categ,input):
    global INDEX_NUM
    df = pd.read_csv("tasks_list_2.csv")
    ...
    try:
        ...
        #go to the next index if the current line is not empty
            ...
            df.to_csv("tasks_list_2.csv", index=False)
...
def back_to_main_from_weeks():
    delete_current_day_tasks("Sunday")
    delete_d_buttons()
    screen_canvas.nametowidget(f'.sun').destroy()
...
def last_sets():
    ...
    screen_canvas.nametowidget(f'.thu').destroy()
    screen_canvas.nametowidget(f'.fri').destroy()
    screen_canvas.nametowidget(f'.sat').destroy()
    delete_current_day_tasks("Thursday")
    delete_current_day_tasks("Friday")
    delete_current_day_tasks("Saturday")
    delete_d_buttons()
    ...
    remove_x = 370
    num = 0
    customtkinter.CTkButton(text="Main Menu", command=back_to_main_from_weeks, text_font=large_font,
                            name="main").place(x=NEXT_BUTTON_X, y=EXAM_BUTTON_Y)
    try:
        task_list = show_task_list("Sunday")
        customtkinter.CTkButton(master=window, text="Sunday", text_font=large_font, name="sun", fg_color="#E74C3C").place(x=168, y=default_y + 60)
        try:
            for task in task_list:
                data_name = str(task.lower().replace(",", "-").replace(".", "-"))
                customtkinter.CTkButton(master=window, text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                                 y=default_y + 100)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",
                                          command=lambda temp=num, temp2=task, temp3="Sunday": delete(temp, temp2,
                                                                                                         temp3),
                                          name=f'd_button{str(num)}', width=53, height=33,
                                          text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 100)
                default_y += 30
                num += 1
        except KeyError:
            pass
...
def next_sets():
    screen_canvas.nametowidget(f'.next').destroy()
    screen_canvas.nametowidget(f'.mon').destroy()
    screen_canvas.nametowidget(f'.tues').destroy()
    screen_canvas.nametowidget(f'.wed').destroy()


    delete_current_day_tasks("Monday")
    delete_current_day_tasks("Tuesday")
    delete_current_day_tasks("Wednesday")
    delete_d_buttons()
    check_var = tkinter.StringVar(value="on")
    read_day_task_data()
    default_y = 100
    remove_x = 370
    num = 0
    customtkinter.CTkButton(text="Show Next", command=last_sets, text_font=large_font,
                            name="next").place(x=NEXT_BUTTON_X, y=EXAM_BUTTON_Y)
    try:
        task_list = show_task_list("Thursday")
        customtkinter.CTkButton(master=window, text="Thursday", text_font=large_font, name="thu", fg_color="#117A65").place(x=168, y=default_y + 60)
            ...
            for task in task_list:
                ...
                customtkinter.CTkButton(master=window, text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                                 y=default_y + 100)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",
                                          command=lambda temp=num, temp2=task, temp3="Thursday": delete(temp, temp2,
                                                                                                      temp3),
                                          name=f'd_button{str(num)}', width=53, height=33,
                                          text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 100)
                default_y += 30
    ...
    remove_x = 370
    try:
        task_list = show_task_list("Friday")
        customtkinter.CTkButton(master=window, text="Friday", text_font=large_font,name="fri",fg_color="#AF601A").place(x=168, y=default_y + 270)
        try:
            for task in task_list:
                data_name = str(task.lower().replace(",", "-").replace(".", "-"))
                customtkinter.CTkButton(master=window, text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                                 y=default_y + 300)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",
                                          command=lambda temp=num, temp2=task, temp3="Friday": delete(temp, temp2,
                                                                                                       temp3),
                                          name=f'd_button{str(num)}', width=53, height=33,
                                          text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 300)
    ...
    remove_x = 370
    try:
        task_list = show_task_list("Saturday")
        customtkinter.CTkButton(master=window, text="Saturday", text_font=large_font,name="sat", fg_color="#3498DB").place(x=168, y=default_y + 470)
        try:
            for task in task_list:
                data_name = str(task.lower().replace(",", "-").replace(".", "-"))
                customtkinter.CTkButton(master=window, text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                                 y=default_y + 500)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",
                                          command=lambda temp=num, temp2=task, temp3="Saturday": delete(temp, temp2,
                                                                                                       temp3),
                                          name=f'd_button{str(num)}', width=53, height=33,
                                          text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 500)
...
def list_weeks_tasks():
    global monday_label
    global tuesday_label, wednesday_label
    check_var = tkinter.StringVar(value="on")
    config_button.destroy()
    pf_button.destroy()
    week_tasks_button.destroy()
    screen_canvas.itemconfig(card_title, text=f"Weekly Task List:", fill="white")
    screen_canvas.itemconfig(card_word, text ="")
    read_day_task_data()
    default_y = 100
    remove_x = 370
    num = 0
    customtkinter.CTkButton(text="Show Next", command=next_sets, text_font=large_font,
                                               name="next").place(x=NEXT_BUTTON_X, y=EXAM_BUTTON_Y)
    try:
        task_list = show_task_list("Monday")
        monday_label = customtkinter.CTkButton(master=window, text="Monday",text_font=large_font, name="mon", fg_color=GREEN).place(x=168, y=default_y + 60)
        ...
        try:
            for task in task_list:
                ...
                customtkinter.CTkButton(master=window,text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                               y=default_y + 100)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",command=lambda temp=num, temp2=task, temp3="Monday": delete(temp, temp2,
                                                                                                       temp3),name=f'd_button{str(num)}', width=53, height=33).place(x=remove_x - 260, y=default_y + 100)
                default_y += 30
                num += 1
        except KeyError:
            pass
    ...
    remove_x = 370
    try:
        task_list = show_task_list("Tuesday")
        tuesday_label = customtkinter.CTkButton(master=window, text="Tuesday",text_font=large_font, name="tues", fg_color="#D4AC0D").place(x=168, y=default_y + 270)
        try:
            for task in task_list:
                data_name = str(task.lower().replace(",", "-").replace(".", "-"))
                customtkinter.CTkButton(master=window,text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                               y=default_y + 300)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",command=lambda temp=num, temp2=task, temp3="Tuesday": delete(temp, temp2,
                                                                                                       temp3),name=f'd_button{str(num)}', width=53, height=33, text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 300)
    ...
    remove_x = 370
    try:
        task_list = show_task_list("Wednesday")
        wednesday_label = customtkinter.CTkButton(master=window, text="Wednesday", text_font=large_font, name="wed").place(x=168, y=default_y + 470)
        try:
            for task in task_list:
                data_name = str(task.lower().replace(",", "-").replace(".", "-"))
                customtkinter.CTkButton(master=window, text=f"{task}",
                                        name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=168,
                                                                                                                 y=default_y + 500)
                customtkinter.CTkCheckBox(text="", variable=check_var, onvalue="on", offvalue="off",
                                          command=lambda temp=num, temp2=task, temp3="Tuesday": delete(temp, temp2,
                                                                                                       temp3),
                                          name=f'd_button{str(num)}', width=53, height=33,
                                          text_font=("roboto", 20)).place(x=remove_x - 260, y=default_y + 500)
...
def read_task_data():
    global task_data
    with open('tasks_list_2.csv', 'r') as infile:
...
def short_term_goals():
    ...
    try:
        ...
        for task in task_list:
            data_name = str(task.lower().replace(",", "-").replace(".", "-"))
            customtkinter.CTkButton(text=f"{task}",
                                    name=f'{data_name}', fg_color="#2C3E50", text_font=("roboto", 20)).place(x=268,
                                                                                                             y=default_y + 100)
            customtkinter.CTkButton(text='delete',
                                    command=lambda temp=num, temp2=task, temp3="long_term": delete(temp, temp2, temp3),
                                    name=f'd_button{str(num)}', width=50, fg_color="#811331", hover_color="#641E16",
                                    text_font=("roboto", 20)).place(x=remove_x - 200, y=default_y + 100)
            default_y += 30
            num += 1
    except KeyError:
        pass
...
def back_to_main():
    ...
    global edit_progress_button, pf_button, generate_html_button,week_tasks_button
...
def day_task_to_csv(categ, input):
    global INDEX_NUM
    df = pd.read_csv("day_task.csv")
    df.dropna(axis=0, how='all', inplace=True)
    df.to_csv("day_task.csv", index=False)
    try:
        entry_data = df.loc[INDEX_NUM, categ]
...
def read_day_task_data():
    global task_data
    with open('day_task.csv', 'r') as infile:
        # read the file as a dictionary for each row ({header : value})
        reader = csv.DictReader(infile)
        task_data = {}
        for row in reader:
