    ...
    try:
        for task in task_list:
            ...
            customtkinter.CTkButton(text='Complete',
                                    command=lambda temp=num, temp2=task, temp3=current_day: delete(temp, temp2, temp3),
...
def show_short_term_goals(current_day):
    ...
    try:
        ...
        task_next_button = customtkinter.CTkButton(text="Main Menu", command=back_to_main,
                                                   text_font=large_font, name="main")
        task_next_button.place(x=NEXT_BUTTON_X - 260, y=EXAM_BUTTON_Y - 300)
        screen_canvas.itemconfig(card_title, text="There are currently no tasks. ", fill="white")
...
def show_long_term_goals(current_day):
    ...
    try:
        ...
        task_next_button = customtkinter.CTkButton(text="Main Menu", command=back_to_main,
                                                   text_font=large_font, name="main")
        task_next_button.place(x=NEXT_BUTTON_X - 260, y=EXAM_BUTTON_Y - 300)
        screen_canvas.itemconfig(card_title, text="There are currently no tasks. Please go back to the Main Menu.", fill="white")
